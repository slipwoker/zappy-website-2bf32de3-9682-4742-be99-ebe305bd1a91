document.addEventListener('DOMContentLoaded', function () {
  ...
  const navbar = document.querySelector('nav, .navbar, [class*="navbar"], [class*="nav-bar"], header');
  ...
  if (contactForm) {
    ...
    function getErrorMessage(field) {
      ...
      const fieldLabel = label ? label.textContent.trim() : name;

      if (field.validity && field.validity.valueMissing) {
        return fieldLabel
          ? 'Please fill in the ' + fieldLabel + ' field.'
    ...
    function validateField(field) {
      ...
      if (field.type === 'checkbox') {
        if (isRequired && !field.checked) {
          showError(field, 'Please check this box to continue.');
          return false;
        }
        clearError(field);
        return true;
      ...
      if (String(value).trim() && field.type in VALIDATORS) {
      ...
      if (field.minLength > 0 && String(value).trim().length < field.minLength) {
  ...
  const OBSERVE_CLASS = 'fade-in-hidden';
  ...
  if (animatableElements.length > 0) {
    animatableElements.forEach(function (el, index) {
      ...
      const delay = el.dataset.animateDelay || (index % 6) * 80;
  ...
  style.textContent = [
    '.fade-in-hidden{opacity:0;transform:translateY(24px);transition:opacity 0.6s ease,transform 0.6s ease;}',
    '.fade-in-visible{opacity:1;transform:translateY(0);}',
    '.input-error{border-color:#e53e3e!important;box-shadow:0 0 0 3px rgba(229,62,62,0.2)!important;}',
    ...
    "translations": {
      "he": {
        "consentModal": {
          ...
          "description": "AquaDrive משתמש בעוגיות כדי לשפר את החוויה שלך, לנתח שימוש באתר ולסייע במאמצי השיווק שלנו.",
...
(function() {
    ...
    contactForm.addEventListener('submit', async function(e) {
        ...
        try {
            ...
            var payload = {
                websiteId: '2bf32de3-9682-4742-be99-ebe305bd1a91',
                name: resolvedName,
                email: resolvedEmail,
            ...
            if (result.success) {
                ...
                if (result.thankYouPagePath && result.ticketNumber) {
                    ...
                    if (isPreview && window.ZAPPY_CONFIG) {
                        var wid = window.ZAPPY_CONFIG.websiteId || '2bf32de3-9682-4742-be99-ebe305bd1a91';
...
(function(){
  try {
    ...
    document.addEventListener('submit', function(e) {
      ...
      var wid = '2bf32de3-9682-4742-be99-ebe305bd1a91';
